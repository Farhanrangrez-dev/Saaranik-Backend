...
const InvoicingBillingCreate = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const ress = await newInvoice.save();

    if (ress) {
  const dd =   await ReceivablePurchase.findByIdAndUpdate(ReceivablePurchaseId, {
        $set: { POStatus: "Received" }
      }, { new: true });
      console.log("Updated ReceivablePurchase:", dd);
    }
    res.status(201).json({
      success: true,
...
// GET Single Invoice with populated details
const GetSingleInvoice = asyncHandler(async (req, res) => {
  const { projectsId, clientId, CostEstimatesId, ReceivablePurchaseId } = req.body;

  // Validate all IDs
  const allProjectIdsValid = Array.isArray(projectsId) && projectsId.every(id => mongoose.Types.ObjectId.isValid(id));
  const isClientIdValid = mongoose.Types.ObjectId.isValid(clientId);
  const isCostEstimateIdValid = mongoose.Types.ObjectId.isValid(CostEstimatesId);
  const isReceivableIdValid = mongoose.Types.ObjectId.isValid(ReceivablePurchaseId);

  try {
    // If projectsId is an array, fetch multiple projects
    const projects = await Projects.find({ _id: { $in: projectsId } });
    const client = await ClientManagement.findById(clientId);
    const costEstimate = await mongoose.model('CostEstimates').findById(CostEstimatesId);
    const receivablePurchase = await mongoose.model('ReceivablePurchase').findById(ReceivablePurchaseId);

    if (!client || !costEstimate || !receivablePurchase) {
      return res.status(404).json({ success: false, message: "One or more records not found" });
    }
...
module.exports = { InvoicingBillingCreate, AllInvoicingBilling, deleteInvoicingBilling, UpdateInvoicingBilling, SingleInvoicingBilling, GetSingleInvoice }