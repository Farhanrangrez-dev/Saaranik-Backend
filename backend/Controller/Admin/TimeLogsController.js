...
//METHOD:GET
const AllTimeLogs = asyncHandler(async (req, res) => {
  try {
    ...
    if (!allTimeLogss || allTimeLogss.length === 0) {
      return res.status(404).json({
        success: false,
        message: "No TimeLogs found in database"
      });
    }

    // Map the data
    const TimeLogssWithDetails = allTimeLogss.map(log => {
      return {
        _id: log._id,
        date: log.date,
        extraHours: log.extraHours,
        hours: log.hours,
        taskNotes: log.taskNotes,
        projects: (Array.isArray(log.projectId) ? log.projectId : []).map(project => ({
          projectId: project?._id,
          projectName: project?.projectName
        })),
        jobs: (Array.isArray(log.jobId) ? log.jobId : []).map(job => ({
          jobId: job?._id,
          jobName: job?.jobName
        })),
      };
    ...
    res.status(500).json({
      success: false,
      message: "An error occurred while fetching TimeLogs",
      error: error.message,
    });
  }
});
